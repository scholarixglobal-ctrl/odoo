...
class CarRentalContract(models.Model):
    ...
    image = fields.Binary(related='vehicle_id.image_128',
                          string="Image of Vehicle", help="Vehicle Image")
    reserved_fleet_id = fields.Many2one(comodel_name='rental.fleet.reserved', copy=False,
                                        help="Reserved Fleet for this Contact")
    name = fields.Char(string="Name", default="Draft Contract",
                       readonly=True, copy=False, help="Name of the contract")
    ...
    vehicle_id = fields.Many2one(comodel_name='fleet.vehicle',
                                 ...
                                 required=True,
                                 help="Vehicle")
    car_brand = fields.Many2one(comodel_name='fleet.vehicle.model.brand',
                                string="Fleet Brand", related='vehicle_id.model_id.brand_id',
                                store=True, readonly=True, help="Manufacture of the fleet")
    car_color = fields.Char(string="Fleet Color", size=50, related='vehicle_id.color', store=True,
                            copy=False, readonly=True, help="Fleet color")
    ...
    start_time = fields.Char(string="Start By", help="Enter the contract starting hour", tracking=True)
    end_time = fields.Char(string="End By", help="Enter the contract Ending hour", tracking=True)
    rent_by_hour = fields.Boolean(string="Rent By Hour", help="Enable to start contract on hour basis")
    rent_end_date = fields.Date(string="Rent End Date", required=True, help="End date of contract", tracking=True)
    state = fields.Selection(
        selection=[('draft', 'Draft'), ('reserved', 'Reserved'), ('running', 'Running'),
         ('cancel', 'Cancel'),
         ('checking', 'Checking'), ('invoice', 'Invoice'), ('done', 'Done')],
        string="State", default="draft", copy=False, tracking=True, help="Contract status")
    notes = fields.Text(string="Details & Notes", help="Additional notes about the contract")
    ...
    journal_type = fields.Many2one(comodel_name='account.journal', string='Journal', help="Journal type to record.",
                                   default=lambda self: self.env['account.journal'].search([('id', '=', 1)]))
    account_type = fields.Many2one(comodel_name='account.account', string='Account', help="Account type to record",
                                   default=lambda self: self.env['account.account'].search([('id', '=', 17)]))
    recurring_line = fields.One2many(comodel_name='fleet.rental.line', inverse_name='rental_number',
                                     readonly=True, help="Recurring Invoices", copy=False)
    first_payment = fields.Float(string='First Payment',  tracking=True, required=True,
                                 help="Transaction/Office/Contract charge amount, must paid by customer side "
                                      "other than recurrent payments")
    first_payment_inv = fields.Many2one(comodel_name='account.move', copy=False, help="First invoice for the contract")
    first_invoice_created = fields.Boolean(string="First Invoice Created", copy=False, help="Show first invoice")
    attachment_ids = fields.Many2many(comodel_name='ir.attachment', relation='car_rent_checklist_ir_attachments_rel',
                                      column1='rental_id', column2='attachment_id', string="Attachments",
                                      help="Images of the vehicle before contract/any attachments")
    checklist_line = fields.One2many(comodel_name='car.rental.checklist', inverse_name='checklist_number',
                                     string="Checklist", help="Facilities/Accessories, That should"
                                          " verify when closing the contract.")
    total = fields.Float(string="Total (Accessories/Tools)", readonly=True,
                         copy=False, help="Total amount of the contract.")
    ...
    damage_cost = fields.Float(string="Damage Cost / Balance Amount", help="damage cost", copy=False)
    damage_cost_sub = fields.Float(string="Damage Cost / Balance Amount", readonly=True, copy=False,
                                   help="Damage sub cost")
    total_cost = fields.Float(string="Total", readonly=True, copy=False, help="Total cost of the contract.")
    invoice_count = fields.Integer(compute='_invoice_count', string='# Invoice', copy=False,
                                   help="Invoice count created")
    check_verify = fields.Boolean(compute='check_action_verify', copy=False, help="Check verify")
    sales_person = fields.Many2one(comodel_name='res.users', string='Sales Person', default=lambda self: self.env.uid,
                                   tracking=True, help="Sales person for the contract")
    read_only = fields.Boolean(string="Read Only", help="To make field read only")
    company_id = fields.Many2one(comodel_name='res.company', string='Company',
                                 default=lambda self: self.env.company, help="Company this record owns")
    ...
    def action_verify(self):
        ...
        if self.total_cost != 0:
            ...
            inv_data = {
                ...
                'move_type': 'out_invoice',
                'partner_id': supplier.id,
                'currency_id': self.account_type.company_currency_id.id,
                'journal_id': self.journal_type.id,
                'invoice_origin': self.name,
                'fleet_rent_id': self.id,
